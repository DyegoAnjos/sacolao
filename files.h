/*A biblioteca "files.h" tem funções que usam struct como parâmetros, será necessário adicionar a struct usada e mudar
o tipo da struct nas funções e caso você não esteja usando uma biblioteca que tenha a sua struct você terá que adicionar
ela aqui no arquivo
*/

/*Sintaxes
	ArqDeletarStc: StcDeletar(Struct usada, tamanho, posição da struct para deletar, ponteiro do arquivo, nome do arquivo, método para abri)
		Deleta uma struct de uma posição, retornando o novo tamanho da struct e escrevendo tudo no arquivo de texto.
	
	ArqEscreverStc: StcEscreverArq(Struct usada, tamanho, ponteiro do arquivo, nome do arquivo, método para abri)
		Escreve toda a struct em um arquivo e retorna 1, caso o arquivo não exista ela retorna -1
		
	ArqLerStc: SrcLerArq(Strct usada, ponteiro do arquivo, nome do arquivo)
